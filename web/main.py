...
@app.route('/superbooking',methods=['GET', 'POST'])
def SUPER():
    if request.method == 'POST':
        return 'Hello ' + request.values['username'] + request.values['userphone'] + request.values['userid'] + request.values['adults']+request.values['children']
...
def getmovietime(moviename, movietheater):
    print(f"{moviename} {movietheater}")
    x = str(moviename)
    y = str(movietheater)
    conn = psycopg2.connect(database=POSTGRES_DB, user=POSTGRES_USER, password=POSTGRES_PW, host=POSTGRES_HOST, port=POSTGRES_PORT)
    cur = conn.cursor()
...
def booking(moviename, movietheater, day, time):
    print(f"{moviename} {movietheater} {day} {time}")
    return render_template('index.html')
