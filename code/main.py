...
def getmovieinfo(moviename):
    ...
    conn = psycopg2.connect(database="DBMS_movie", user="postgres", password=IN1, host="127.0.0.1", port="5432")
    cur = conn.cursor()
    sql = "select actor from acts where movie_name = %r" %x
    cur.execute(sql)
    uact = cur.fetchall()
    conn.close()
    ...
    conn.close()
    if len(t) == 0:
        return render_template('comingsoon.html', act=uact, U=u, T=x, t=(("即將上映",),), I=info)
    return render_template('movie.html', act=uact, U=u, T=x, t=t, I=info)
